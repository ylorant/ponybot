...
class Config
{
	...
	{
		Ponybot::message('Loading config...');
		if(!is_dir($this->_configDirectory))
		{
			Ponybot::message('Could not access to user-set confdir : $0', array($this->_configDirectory), E_WARNING);
			
			//Throw a fatal error if the default config directory does not exists
			if(!is_dir('conf'))
			{
				Ponybot::message('Could not access to default confdir : ./conf', array(), E_ERROR);
				exit();
			}
			else
	...
	public function parseCFGDirRecursive($dir)
	{
		...
		foreach($dirContent as $file)
		{
			...
			elseif(is_dir($dir.'/'.$file) && !in_array($file, array('.', '..')) && $file[0] != '.')
			{
				...
				{
					Ponybot::message('Parse error in $0 directory.', array($dir), E_WARNING);
					return FALSE;
				}
			}
		}
	...
	public function setConfig($section, $data)
	{
